...
function calc() {
    ...
    let sex, heiht, weight, age, ratio;
    ...
    function calcTotal() {
        if (!sex || !heiht || !weight || !age || !ratio) {
            result.textContent = '____';
            return;
        }
        if (sex === 'female') {
            result.textContent = Math.round((447.6 + (9.2 * weight) + (3.1 * heiht) - (4.3 * age)) * ratio);
        } else {
            result.textContent = Math.round((88.36 + (13.4 * weight) + (4.8 * heiht) - (5.7 * age)) * ratio);
        }
    ...
    function getDynamicInformation(selector) {
        ...
        input.addEventListener('input', () => {
            ...
            if (input.value.match(/\D/g)) {
                input.style.cssText = `box-shadow:0 4px 15px rgba(255,0,0,.8)`;
            } else {
                input.style.cssText = `box-shadow:0 4px 15px rgba(0,0,0,.2)`;
            }
    ...
    (0,_services_services__WEBPACK_IMPORTED_MODULE_0__.getResource)('http://localhost:3000/menu')
        .then(data => {
            data.forEach(({img, altimg, title, descr, price}) => {
                new AddMenu(img, altimg, title, descr, price, '.menu .container').render();
            });
        });
...
window.addEventListener('DOMContentLoaded', () => {
        ...
        _modules_timer__WEBPACK_IMPORTED_MODULE_5___default()('.timer', '2022-12-20');
